...
To facilitate the comparison of the arrival delays for each airline, we split our table into two tables as follows by using the subset command.

```{r}
Alaskaarrivaldelays = subset(arrivaldelays, Airline == "Alaska", select = -Airline)
AMWestarrivaldelays = subset(arrivaldelays, Airline == "AM West", select = -Airline)
Alaskaarrivaldelays
AMWestarrivaldelays
```

Then, we add a new column to each table and title it performance. It is computed as the percentage of flights that arrive on time. 

```{r}
Alaskaarrivaldelays$performance = round((Alaskaarrivaldelays$punctual/(Alaskaarrivaldelays$punctual + Alaskaarrivaldelays$delayed))*100, 0)
AMWestarrivaldelays$performance = round((AMWestarrivaldelays$punctual/(AMWestarrivaldelays$punctual + AMWestarrivaldelays$delayed))*100, 0)
Alaskaarrivaldelays
AMWestarrivaldelays
```
...
We now compute the mean performance for each airline as follows.

```{r}
round(mean(Alaskaarrivaldelays$performance), 0)
round(mean(AMWestarrivaldelays$performance), 0)
```
